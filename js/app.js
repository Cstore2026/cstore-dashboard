...
function tabsFor(){if(!current)return[];if(current.role==='admin')return ['overview','orders','prepAdmin','deliveryAdmin','completedOrders','delivery','people','accounts','settings'];if(current.role==='ccadmin')return ['overview','orders','ccAdd','ccManage','ccReport'];if(current.role==='callcenter')return ['callcenter','orders','completedOrders'];if(current.role==='branch')return ['branchNewOrders','orders','branchAssignRider','completedOrders'];if(current.role==='rider')return ['riderOrders'];return ['overview']}
