...
function completedOrdersHTML(){let list=filterOrdersRange(visibleOrders(),'completed');return `<div class="card"><div class="row" style="justify-content:space-between"><div><h3>${lang==='ar'?'متابعة حركة الأوردرات':'Order Live Tracking'}</h3><p class="muted">${lang==='ar'?'كل الأوردرات تظهر هنا فورًا لكل اليوزرات مع كل مراحل الحركة والأوقات.':'All orders appear here immediately for all users with full movement and timings.'}</p><p class="note">${lang==='ar'?'مكان اللوكيشن هيظهر هنا بعد ربط الأبلكيشن: موقع الطيار عند ضغط تم التوصيل أو الرفض / الإلغاء.':'Location will appear here after app integration: rider position when pressing delivered or refused / cancelled.'}</p></div><button class="blue" onclick="exportCompletedCSV()">${tr('exportExcel')}</button></div>${completedFilterControls()}${allTimesTable(list,false)}</div>`}

function ordersHTML(){let isAdmin=current&&current.role==='admin';let isCcAdmin=current&&current.role==='ccadmin';let isCallCenter=current&&current.role==='callcenter';let isBranch=current&&current.role==='branch';let title=isAdmin?tr('allTimes'):((isCallCenter||isBranch||isCcAdmin)?(lang==='ar'?'متابعة كل الأوردرات':'Track All Orders'):tr('orders'));let hint=isAdmin?`<p class="muted">${tr('allTimesHint')}</p>`:'';let baseList=isCcAdmin?state.orders.filter(o=>o.ccStaff):visibleOrders();let list=isCcAdmin?filterOrders(baseList,'ccOrders'):baseList;let table=isAdmin?allTimesTable(list):ordersTable(list,isCcAdmin?false:true);let filtersHTML=isCcAdmin?filterControls('ccOrders'):'';return `<div class="card"><div class="row" style="justify-content:space-between"><div><h3>${title}</h3>${hint}</div><button class="blue" onclick="exportCSV()">${lang==='ar'?'تحميل CSV':'Export CSV'}</button></div>${filtersHTML}${table}</div>`}
